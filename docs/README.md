...
|---|---|---|
| `research/macos.md` | written, review outstanding | how macOS stores and applies wallpaper per display and per Space, what the supported API is, multi-display and hotplug behaviour, minimum OS version |
| `research/windows.md` | written, review outstanding | the same for Windows 10/11, including whether per-monitor and per-virtual-desktop wallpaper are possible at all |
| `research/linux.md` | written, review outstanding | the per-desktop-environment matrix (GNOME, KDE, sway, Hyprland, X11) and which compositors require a resident process |
| `research/scheduling.md` | written, review outstanding | launchd, Task Scheduler and systemd user timers: missed-run behaviour across sleep, install/uninstall, and whether they can replace an in-daemon timer |
| `spec/features.md` | written, review outstanding | the v0.1 feature set, the source abstraction, filters, and what is explicitly out of scope |
| `spec/state-and-cache.md` | written, review outstanding | per-OS state and cache locations, eviction policy, atomic writes, corruption recovery |
...
| `reviews/` | empty | one review report per reviewed document: what the reviewer ran, what they observed, what they could not check |
