...
| On a two-display machine on a platform that honours `per-display`, a per-display set touches one screen and leaves the other alone. | Not this machine, and not macOS at all: the route is a sway session or a real Windows desktop, two displays attached, driving the backend's per-screen set directly rather than a rotation. `swaymsg output <output> bg <file> fill` names one output (`docs/research/linux.md`); `IDesktopWallpaper::SetWallpaper(monitorID, path)` takes one id from `GetMonitorDevicePathAt` (`docs/research/windows.md`). The named screen shows the new image and the other shows what it showed before. Reading back per screen is available on one of the two routes only: Windows has an image readback, `GetWallpaper(monitorID)` returning that monitor's image (`docs/research/windows.md`:83), and sway has none: `swaymsg -t get_outputs` reports output metadata and no background key (`ipc_json_describe_wlr_output` in sway's `sway/ipc-json.c`, sway master, read while this file was written), which is why `docs/research/linux.md`:265-266 cites it for the make/model/serial addressing string alone, and `docs/spec/features.md` 1.5 carries reading the image back as an `assumption:` that is "not confirmed on every platform". The sway readback is therefore the child sway owns, `pgrep -a swaybg` (the live check at `docs/research/linux.md`:307-308): sway builds its argv as one `-o <output>` / `-i <image>` / `-m <mode>` triple per output (`spawn_swaybg` in sway's `sway/config/output.c`, sway master, read while this file was written), so the triples name the image each output holds, the untouched one included. macOS cannot carry this criterion on either reading of row 1: `per-display` there is accepted and runs as `all`, so `set()` puts the same image on every screen (`crates/whirl-worker/src/backend/macos.rs`, `fn set` at 398), and `scripts/desktop-snapshot.sh` and `scripts/desktop-restore.sh` are macOS-only, so they are not this check's harness. It cannot pass on a one-display machine either, whatever the platform, and is expected to block there. Artifact: on Windows, `GetWallpaper` per monitor id; on sway, the one `pgrep -a swaybg` line, whose triples are the per-output readback. |
| The desktop is left exactly as it was found. | `scripts/desktop-restore.sh` after the check, and the restore proved by reading the store back and printing the image line. Those two scripts are macOS-only, so on the platforms row 2 names the restore is that platform's own set back to the snapshot image, read back with that platform's evidence from row 2. A test fixture left as the desktop picture is a failed check, not a pass. |
