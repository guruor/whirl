...
Every script named in the table is a committed file under `docs/research/probes/`. The `.plist`
snapshots the commands read (`$OUT/w0.plist`, `$OUT/w1.plist`, `$OUT/w2.plist`, `l0.plist`,
`l1.plist`, `before.plist`) are captured at run time into a scratch directory and are deliberately
not committed, because they carry this machine's wallpaper history; the recipes in the README show
...
| [V5] | `sh allspaces_test.sh` (write with `allSpaces`, write without, restore), recipe in `probes/README.md` | `changed (choice or LastSet): 2` both ways, the same 2 nodes; `LastUse` churn on 21 slots / 9 Space UUIDs. Re-ran 2026-09-25 14:37 IST: same 2 nodes both ways, `added=0 removed=0`, `LastSet` 2 of 2409, `LastUse` 21 of 2409, live image restored and confirmed by `wp_probe` readback |
| [V5b] | `python3 churn2.py $OUT/w0.plist $OUT/w1.plist` and `python3 lastuse_groups.py $OUT/w0.plist $OUT/w1.plist`, on the snapshots `allspaces_test.sh` writes into its scratch dir | `LastSet: 2 of 2409`, `LastUse: 21 of 2409`, 10 distinct timestamps inside 1.2 ms. Re-ran 2026-09-25: same 2 of 2409 and 21 of 2409, 10 distinct timestamps, spread 1.67 ms (the spread is scheduler jitter, not a fixed constant) |
| [V5c] | `./wp-set <deleted-path> allspaces` | `screen M27Q -> FAILED error=The file doesn't exist.`, store unchanged |
| [V5d] | `sh inode_test.sh` | inode `203393620` to `203394346` across one write, directory listing never shows a temp file: the store is replaced, not edited. Re-ran 2026-09-25: inode `203464416` to `203464586` across one write, same directory listing before and after (the inode numbers differ every run because a new file is written each time; the change is the finding) |
| [V6] | `python3 live_spaces.py` | `LastUse` age buckets: `<1h 0`, `<24h 9`, `<7d 12`, `<30d 35`, `<1y 403`, `older 16`, total 475. Re-ran 2026-09-25 14:07 IST: `<1h 9`, `<24h 0`, `<7d 12`, `<30d 36`, `<1y 403`, `older 15`, total 475. The buckets are relative to now, so the same 9 in-use Spaces sit in `<1h` rather than `<24h` and the aging bins creep by a day; the in-use count, and the finding that 466 of 475 nodes are historic leftovers, is stable |
| [V7] | `launchctl submit -l com.whirl.probe4 -- /bin/sh probe-wrapper.sh` and `... -- /bin/sh set-wrapper.sh` (committed wrappers over `wp_probe` / `wp_set`); then `launchctl list`; then `raw_node.py` on `$OUT/l0.plist` and `$OUT/l1.plist` | `LimitLoadToSessionType = "Aqua"`; launchd-run read saw `NSScreen.screens count: 1`; launchd-run write returned `OK` and bumped `LastSet` 07:21:11 to 07:22:29; `launchctl list \| grep whirl` afterwards: none. Re-ran 2026-09-25: `launchctl submit` reported `LimitLoadToSessionType = "Aqua"` with `LastExitStatus = 0`; the launchd-run read saw 1 screen; the launchd-run write of a different image returned `OK` and bumped `LastSet` on the live Space node (`changed (choice or LastSet): 2`, `LastSet: 2 of 2409`); `launchctl remove` left no `com.whirl` job. A same-image re-write is a no-op in the store, so the run that registers must write a different image |
| [V8] | `python3 probes/wallpaper_store.py dump` (the `dump` subcommand; earlier drafts split this across two throwaway scripts that are no longer in the tree) | the decoded tree in section 1; 0 hits for `Appearance`; `AllSpacesAndDisplays` has no `Desktop` slot |
| [V9] | `sh sandbox_test.sh` (build with `com.apple.security.app-sandbox`, ad-hoc sign, run) then the control without the entitlement | sandboxed: `Trace/BPT trap: 5`, AMFI `-423 "The file is adhoc signed or signed by an unknown certificate chain"`; control: runs, `OK`, exit 0. Re-ran 2026-09-25: sandboxed `Trace/BPT trap: 5` (exit 133) with `AppSandbox` in `libsystem_secinit`, kernel `AMFI: '/private/tmp/wp_set_sandbox' is adhoc signed.`, amfid `-423`; control `OK`, exit 0. The crash report the trap leaves was deleted afterwards |
