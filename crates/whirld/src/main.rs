...
//! `subscribe` stream of docs/architecture.md 2.9, the schedule of 5.5 and the
//! cache's own index and sweep of section 5 are real, and the wallpaper backends
//! are later cards. It logs to stderr, because a scaffold is run in the
//! foreground (docs/development.md section 7) and the log file of `[D 6 §7.2]`
//! arrives with the daemon that has somewhere to put it.
...
fn run(flags: &plan::Flags) -> Result<(), String> {
    ...
    // while the walk runs is answered from the state that is already loaded; the
    // scheduler's first slot starts below, so a seed over either cap is corrected
    // before any download can add to it.
    daemon.sweep();
